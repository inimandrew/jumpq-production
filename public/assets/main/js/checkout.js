        ...
        $.ajax({
            ...
            if (carts.length > 0) {
                jQuery.each(carts, function (i, val) {
                    var tr = $("<tr></tr>");
                    var td = $("<td></td>").addClass(
                        "border border-black py-4 text-center"
                    );
                    ...
                    $(tr)
                        .append(
                            ...
                            //             .attr("type", "button")
                            //             .addClass(
                            //                 "btn btn-xs btn-circle btn-secondary remove"
                            //             )
                            //     )
                            //     .clone(),
                            $(td)
                                .html(
                                    $("<img>")
                                        .attr("src", val.thumbnail.location)
                                        .addClass(" mx-auto")
                                )
                                .clone(),
                            ...
                            $(td)
                                .text(
                                    val.currency + Number(val.price).toFixed(2)
                                )
                                .clone(),
                            ...
                            $(td)
                                .text(
                                    val.currency +
                                        Number(product_total).toFixed(2)
                                )
                                .clone()
                        )
