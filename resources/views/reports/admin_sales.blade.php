    ...
    <table border="1">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{formatNumber($transaction->service_charge,2,'.',',')}}
                </td>
                <?php $new_profit = calculateProfit($transaction->sales);
                      $total_profit += $new_profit;
                      $total += ($transaction->total + $transaction->service_charge);
                 ?>
                <td>{{formatNumber($total,2,'.',',')}}</td>
                <td>{{formatNumber($new_profit)}}</td>
            </tr>
            @endforeach
            <tr>
                <td colspan="9"></td>
