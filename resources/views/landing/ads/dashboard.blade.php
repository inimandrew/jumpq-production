...
<div class=" w-full h-full bg-white">
    ...
    <div class="grid grid-rows-1 gap-4 md:grid-cols-3 mx-auto w-11/12 md:w-3/5 py-6">
        <div class=" px-4 py-4 border-2 border-black color-app rounded-md">
            ...
            <span class="block text-right">{{$campaigns->count()}}</span>
        </div>

        <div class=" px-4 py-4 border-2 border-black color-app rounded-md">
            <span class="block text-left">Approved Campaigns</span>
            <span class="block text-right">{{$campaigns->where('approved','1')->count()}}</span>
        </div>

        <div class=" px-4 py-4 border-2 border-black color-app rounded-md">
            <span class="block text-left">Rejected Campaigns</span>
            <span class="block text-right">{{$campaigns->where('approved','2')->count()}}</span>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border border-gray-600 px-2 py-2 text-sm text-center">
                        ...
                        <span
                            class="border border-blue-400 text-white bg-blue-400 rounded-full py-1 px-2 text-xs">Inactive</span>
                        @else
                        <span
                            class="border border-green-400 text-white bg-green-400 rounded-full py-1 px-2 text-xs">Active</span>
                        @endif
                    </td>
                    <td class="border border-gray-600 px-2 py-2 text-sm text-center">
                        @if ($campaign->paid == '0')
                        <span
                            class="border border-blue-400 text-white bg-blue-400 rounded-full py-1 px-2 text-xs">Awaiting
                            Payment</span>
                        @else
                        <span
                            class="border border-green-400 text-white bg-green-400 rounded-full py-1 px-2 text-xs">Payment
                            Recieved</span>
                        @endif
                    </td>
                    ...
                    <td class="border border-gray-600 px-2 py-2 text-sm text-center">
                        ...
                        <span
                            class="border border-blue-400 text-white bg-blue-400 rounded-full py-1 px-2 text-xs">Waiting
                            Approval</span>
                        @elseif($campaign->approved == 1)
                        <span
                            class="border border-green-400 text-white bg-green-400 rounded-full py-1 px-2 text-xs">Approved</span>
                        @elseif($campaign->approved == 2)
                        <span
                            class="border border-red-400 text-white bg-red-400 rounded-full py-1 px-2 text-xs">Rejected</span>
                        @endif
                    </td>
                    ...
                    <td class="border border-gray-600 px-2 py-2 text-sm text-center">
                        ...
                        <button data-amount="{{$campaign->amount}}" data-id="{{$campaign->id}}"
                            data-email="{{$campaign->account->email}}" data-name="{{$campaign->account->company_name}}"
                            type="button" class="text-white bg-green-400 py-2 px-2 paystack rounded-md"
                            data-key="{{$data['paystack']->value}}"> Make Payment</button>
                        @elseif($campaign->payment_type_id == 4)
                        <button data-amount="{{$campaign->amount}}" data-id="{{$campaign->id}}"
                            data-email="{{$campaign->account->email}}" data-name="{{$campaign->account->company_name}}"
                            type="button" class="text-white bg-green-400 py-2 px-2 flutter rounded-md"
                            data-key="{{$data['flutter']->value}}"> Make Payment</button>
                        @endif
                        @endif
                    </td>
